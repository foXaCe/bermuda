...
* bound cycle time by skipping dormant-device processing ([#117](https://github.com/foXaCe/bermuda/issues/117)) ([e78a033](https://github.com/foXaCe/bermuda/commit/e78a033cd9aef1512408d4597babd95e626a8d18))

## [0.14.1](https://github.com/foXaCe/bermuda/compare/v0.14.0...v0.14.1) (2026-08-09)


### Changed

* speed up startup via lazy imports and fix configured-device pruning ([5a9869d](https://github.com/foXaCe/bermuda/commit/5a9869d995b0777981f45396da87ddf2d2a41115))
...
- **Perf (fork deviation, documented in `const.py`):** the update cycle no longer runs the distance smoothing (`calculate_data`) over every discovered device each ~1 s. Only devices that are actually relevant are processed: anything seen on the air that cycle, plus tracked devices, scanners, metadevices, metadevice sources and configured-but-unseen devices. Dormant untracked noise (the MACs the proxies see once and never again) is skipped entirely. Benchmarked on a 617-device population (13 tracked + 600 dormant): Phase 3 dropped from 2.33 ms to 0.12 ms per cycle (~19x)
- **Perf (fork deviation, documented in `const.py`):** `PRUNE_TIME_DEFAULT` lowered from 86400 s (24 h) to 7200 s (2 h) — an untracked static MAC that is never a metadevice source is pure noise, so retaining it for a day only grows the per-cycle passes and memory. Configured devices, scanners, metadevices and metadevice sources are still protected by dedicated pruning keepers; IRK/Private-BLE timeouts are unchanged. `PRUNE_MAX_COUNT` lowered from 1000 to 500 for the same domestic-scale reasoning (no effect on trilateration — pruned entries are non-tracked noise only)
- The update cycle now records a per-phase timing breakdown (`coordinator.cycle_stats`), logs it on every cycle at DEBUG and alongside the existing WARNING/ERROR when a cycle is slow, and exposes it in diagnostics — so where cycle time actually goes is measurable on a live instance
